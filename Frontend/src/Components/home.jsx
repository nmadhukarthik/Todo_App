...
const Home = () => {
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      try {
        setLoading(true)
        const response = await axios.get('http://localhost:4002/todo/fetchTodos', {
  ...
  return (
    <div className='mt-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
      ...
      <div className='flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 m-4'>
        <input className='flex-grow p-2 border rounded focus:outline-none'
        ...
        <button className='bg-blue-600 border rounded text-white px-4 py-2 hover:bg-blue-900 duration-300' 
                onClick={() => createTodo()}> 
                Add
        </button>
      </div>
            ...
            todos.map((todo, index) => (
              <li key={todo._id || index} className='flex items-center justify-between  p-3 bg-gray-100 rounded-md '>
                <div className='flex flex-1 items-center'>
                  ...
                  <span className={`${todo.completed ? "line-through text-gray-900  font-semibold" : ""} `}> {todo.title} </span>
                </div>
                <span className={`${todo.completed ? "line-through text-gray-900  font-semibold" : ""} flex-1 text-center `}> 
                      {todo.date} 
                </span>
                <button className='text-red-500 hover:text-red-800 flex-1 text-right '
