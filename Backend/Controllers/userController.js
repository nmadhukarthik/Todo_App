...
export const signup = async(req,res) => {
    try {
        ...
        const validate = userSchemaValidation.safeParse({username, email, password})
        
        if(!validate.success)
        { 
            const errorMessage = validate.error.errors.map((error)=>error.message)
        ...
        const newUser = new userSchemaModal({username, email, password:hashPassword})
        await newUser.save()
        if(newUser)
        { 
...
export const login = async(req,res) => {
    ...
    try {
        ...
        const user = await userSchemaModal.findOne({email}).select("+password")
...
export const logout = (req,res) => {
    try {
        ...
        console.log(error)
        return res.status(400).json({message: "Error in logging out the user"})
    }
}